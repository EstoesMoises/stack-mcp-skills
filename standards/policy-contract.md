...
- Never blindly retry an ambiguous write outcome or a write whose response is lost. First reconcile current state read-only, including duplicate or target-state retrieval appropriate to the action; then rebuild and redisplay the complete exact payload, action, target, and arguments and obtain fresh explicit approval immediately before every retry, even when nothing changed. Prior approval is never reusable for a retry.
