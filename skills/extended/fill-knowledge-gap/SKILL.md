...
- If a write fails, its outcome is ambiguous, or its response is lost, never blindly retry or reuse prior approval. First reconcile current state read-only with the complete three-search duplicate check and full retrieval of any possible new question. Before every retry, rebuild and redisplay the complete exact question payload, action, and arguments, then obtain fresh explicit approval immediately before the call even when nothing changed.
