...
- If a write fails, its outcome is ambiguous, or its response is lost, never blindly retry or reuse prior approval. First reconcile current state read-only by retrieving the complete question, answers, accepted-answer state, and duplicate/current vote-relevant state available from the live schema. Before every retry, rebuild and redisplay the complete exact payload, evidence, target, action, and arguments, then obtain fresh explicit approval immediately before the call even when nothing changed.
