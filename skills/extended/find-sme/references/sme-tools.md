...
2. `get_existing_tags` resolves topic language to existing tags. The supported catalog schema has no filter arguments: call it with an empty argument object and filter the returned tag names and IDs locally. Pass a server-side filter only when the inspected live schema declares one. Select only an exact semantic match, and ask the user to choose when more than one tag is plausible.
