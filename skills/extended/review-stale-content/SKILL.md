...
3. Fetch the complete discussion before classifying a question or an answer. For a question, call `get_question` and call `get_comments` for the question and for each answer under consideration. For an article, use `get_article`, which returns the full article with its comments. Articles are review-only because no article update tool is declared: a confirmed stale article reports its divergence without an approval-ready update. Retrieval or comments failure makes the review incomplete; do not use a search snippet, an unseen answer, or an unseen comment as evidence.
