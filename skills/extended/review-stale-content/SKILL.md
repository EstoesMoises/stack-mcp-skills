...
3. Fetch the complete discussion before classifying a question or an answer. For a question, call `get_question` and call `get_comments` for the question and for each answer under consideration. For an article, use `get_article`, which returns the full article with its comments. Retrieval or comments failure makes the review incomplete; do not use a search snippet, an unseen answer, or an unseen comment as evidence.
...
Approval covers only the unchanged client payload, target IDs, action, and arguments. Any material edit, source interpretation, classification, target, action, schema mapping, or argument change requires redisplaying the complete payload and obtaining approval again. After approval, replay `intended_action.args` byte-for-byte with the approved update tool. Never add defaults, transform content, infer a field, substitute a target, or switch actions after approval.
...
- If an approved update fails or lacks confirmation, preserve the approved payload and report that no update was confirmed. Require explicit approval again after any retry-relevant change.
