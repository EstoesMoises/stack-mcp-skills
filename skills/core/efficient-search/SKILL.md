...
4. Rank promising candidates strongest-first from their titles and tags. Retrieve the strongest question with `get_question` or article with `get_article` before relying on it, then reassess whether its full content is sufficient. Retrieve the next candidate only when the evidence remains insufficient; do not fetch every promising candidate eagerly.
