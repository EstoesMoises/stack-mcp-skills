...
- If a write fails, its outcome is ambiguous, or its response is lost, never blindly retry or reuse prior approval. First reconcile current state read-only with duplicate search and full retrieval of any possible created article or Q&A. If the exact approved write already succeeded, report the confirmed result and stop without redisplay, approval, or retry. If reconciliation is inconclusive and a retry remains necessary, rebuild and redisplay the complete exact incident payload, action, and arguments, then obtain fresh explicit approval immediately before the call even when nothing changed.
